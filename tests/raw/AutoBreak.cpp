    ...
    AutoBrake(IServiceBus& bus) 
      : collision_threshold_s{ 5 },
        speed_mps{} {
          ...
          bus.subscribe([this, &bus](const CarDetected& car) {
            const auto relative_speed_mps = speed_mps - car.speed_mps;
            const auto time_to_collision_s = car.distance_m / relative_speed_mps;
...
/**
 * This is the test harness, responsible for running a test unit
 */
...
int main() {
  ...
  run_test(initial_sensitivity_is_five, "initial sensitivty is 5");
  ...
  run_test(alert_when_imminent, "brake if imminent collision");
}
